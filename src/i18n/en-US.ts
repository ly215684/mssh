/* English dictionary */
export default {
  // Common
  'common.cancel': 'Cancel',
  'common.save': 'Save',
  'common.connect': 'Connect',
  'common.close': 'Close',
  'common.delete': 'Delete',
  'common.rename': 'Rename',
  'common.refresh': 'Refresh',
  'common.confirm': 'Confirm',
  'common.edit': 'Edit',
  'common.copy': 'Copy',
  'common.ok': 'OK',
  'common.name': 'Name',
  'common.loading': 'Loading…',
  'common.empty': 'No data',
  'common.lines': 'lines',

  // Sidebar
  'sidebar.searchPh': 'Search servers',
  'sidebar.quickConnect': 'Quick Connect',
  'sidebar.connections': 'Connections',
  'sidebar.add': 'Add',
  'sidebar.newConnection': 'New Connection',
  'sidebar.newGroup': 'New Group',
  'sidebar.darkMode': 'Dark Mode',
  'sidebar.lightMode': 'Light Mode',
  'sidebar.settings': 'Settings',
  'sidebar.noResults': 'No results',
  'sidebar.ungrouped': 'Ungrouped',

  // Context menu
  'ctx.openTerminal': 'Open Terminal',
  'ctx.connect': 'Connect',
  'ctx.edit': 'Edit',
  'ctx.duplicate': 'Duplicate',
  'ctx.deleteConn': 'Delete Connection',
  'ctx.deleteGroup': 'Delete Group',
  'ctx.renameGroup': 'Rename Group',
  'ctx.upload': 'Upload',
  'ctx.download': 'Download',
  'ctx.mkdir': 'New Folder',
  'ctx.touch': 'New File',
  'ctx.extract': 'Extract Here',
  'ctx.moveTo': 'Move to…',
  'ctx.deleteFile': 'Delete',
  'ctx.open': 'Open',
  'editor.title': 'Edit Remote File',
  'editor.loading': 'Loading…',
  'editor.saving': 'Saving…',
  'editor.saved': 'Saved',
  'editor.save': 'Save',
  'editor.cancel': 'Cancel',
  'editor.close': 'Close',
  'editor.dirtyConfirm': 'The file has unsaved changes. Close anyway?',

  // New connection
  'newConn.title': 'New Connection',
  'newConn.titleEdit': 'Edit Connection',
  'newConn.name': 'Name',
  'newConn.namePh': 'e.g. Production Server',
  'newConn.host': 'Host',
  'newConn.hostPh': '192.168.1.100',
  'newConn.port': 'Port',
  'newConn.username': 'Username',
  'newConn.authType': 'Authentication',
  'newConn.password': 'Password',
  'newConn.privateKey': 'Private Key',
  'newConn.keyPath': 'Private key file',
  'newConn.browse': 'Browse',
  'newConn.passphrase': 'Key passphrase (optional)',
  'newConn.group': 'Group',
  'newConn.groupNone': 'None',

  // Quick connect
  'quick.title': 'Quick Connect',
  'quick.target': 'Target',
  'quick.targetPh': 'user@host:port or host',
  'quick.password': 'Password',
  'quick.hint': 'Quick connections are not saved to the list',

  // Terminal
  'term.upload': 'Upload',
  'term.download': 'Download',
  'term.disconnect': 'Disconnect',
  'term.connecting': 'Connecting to {target}…',
  'term.connClosed': 'Connection closed',
  'term.connFailed': 'Connection failed: {msg}',
  'term.connFailedTitle': 'Connection Failed',
  'term.reconnect': 'Click here to reconnect',
  'term.autoRetrying': 'Auto-reconnecting (attempt {n}/{max})',
  'term.stopRetry': 'Stop auto-reconnect',
  'term.sftp': 'SFTP',
  'term.ai': 'AI Assistant',
  'term.docker': 'Docker',
  'term.cron': 'Scheduled Tasks',

  // Docker
  'docker.containers': 'Containers',
  'docker.images': 'Images',
  'docker.compose': 'Compose Up',
  'docker.pull': 'Pull Image',
  'docker.pullTitle': 'Pull Image',
  'docker.pullHint': 'Enter an image name (e.g. nginx:latest, library/ubuntu). Runs docker pull to fetch it locally.',
  'docker.pullPlaceholder': 'nginx:latest',
  'docker.pullRunning': 'Pulling image, may take a few minutes…',
  'docker.pullDone': 'Image pulled successfully',
  'docker.pullFailed': 'Pull failed, check the output below',
  'docker.invalidImage': 'Please enter an image name',
  'docker.run': 'Run Container',
  'docker.runTitle': 'Run Container from Image',
  'docker.runHint': 'Create and start a new container from an image. Configure ports / env / volumes below.',
  'docker.runName': 'Container Name',
  'docker.runNamePlaceholder': 'auto-generated if empty',
  'docker.runPorts': 'Port Mapping',
  'docker.runPortsHint': 'One per line, host:container or host:container/tcp (e.g. 8080:80)',
  'docker.runEnv': 'Environment',
  'docker.runEnvHint': 'One per line, KEY=VALUE',
  'docker.runVolumes': 'Volumes',
  'docker.runVolumesHint': 'One per line, host:container (e.g. /data:/var/lib/mysql)',
  'docker.runDetach': 'Detached (-d)',
  'docker.runCmd': 'Command',
  'docker.runCmdPlaceholder': 'Optional, overrides image CMD, e.g. sh -c "echo hi"',
  'docker.runExtraArgs': 'Extra Args',
  'docker.runExtraArgsHint': 'Optional, shell fragment appended to docker run, e.g. --restart=always --network=host',
  'docker.runExtraArgsPlaceholder': '--restart=always --network=host',
  'docker.runRunning': 'Starting container, pulling image may take a few minutes…',
  'docker.runDone': 'Container started',
  'docker.runFailed': 'Failed to start container, check the output below',
  'docker.invalidPort': 'Invalid port mapping: {value}',
  'docker.invalidEnv': 'Invalid env format: {value}',
  'docker.colName': 'Name',
  'docker.colImage': 'Image',
  'docker.colStatus': 'Status',
  'docker.colPorts': 'Ports',
  'docker.colRepo': 'Repository',
  'docker.colTag': 'Tag',
  'docker.colId': 'Image ID',
  'docker.colSize': 'Size',
  'docker.noContainers': 'No containers',
  'docker.noImages': 'No images',
  'docker.ctxHint': 'Right-click a container to start / stop / restart / view logs / delete; double-click for logs. Right-click an image to run a container or delete the image',
  'docker.remove': 'Delete',
  'docker.removeContainerTitle': 'Delete container',
  'docker.removeContainerConfirm': 'Delete container "{name}"?',
  'docker.removeContainerRunningConfirm': 'Container "{name}" is running and will be force-stopped and deleted. Continue?',
  'docker.removeImageTitle': 'Delete image',
  'docker.removeImageConfirm': 'Delete image {repo}:{tag}?',
  'docker.removeDone': 'Deleted {name}',
  'docker.opRunning': 'Running operation…',
  'docker.allLogs': 'All logs',
  'docker.errUnavailable': 'Docker is unavailable',
  'docker.errPerm':
    'The current user has no Docker access: add the user to the docker group (sudo usermod -aG docker $USER, then log in again) or connect as root.',
  'docker.errInstall':
    'Docker was not found on the server. Install it first (curl -fsSL https://get.docker.com -o get-docker.sh && sudo sh get-docker.sh).',
  'docker.install': 'Install Docker',
  'docker.installTitle': 'Install Docker',
  'docker.installDetecting': 'Detecting server system…',
  'docker.installDetected': 'Detected system',
  'docker.installDetectFailed': 'Failed to detect server system',
  'docker.installPrivilege': 'Privilege',
  'docker.installRoot': 'Running as root, will install directly',
  'docker.installSudo': 'Not root, will install via sudo (passwordless sudo required)',
  'docker.installNoSudo': 'Not root and sudo unavailable. Please connect as root and retry.',
  'docker.installHint': 'This will download and install Docker Community Edition. The process may take 1-3 minutes. Do not close the window.',
  'docker.installCmd': 'Install command',
  'docker.installRun': 'Start Install',
  'docker.installRunning': 'Installing…',
  'docker.installDone': 'Docker installed successfully',
  'docker.installFailed': 'Docker installation failed, check the output below',
  'docker.installUnsupported': 'Unsupported system: {name}. Please install Docker manually.',
  'docker.start': 'Start',
  'docker.stop': 'Stop',
  'docker.restart': 'Restart',
  'docker.logs': 'View Logs',
  'docker.startTitle': 'Start Container',
  'docker.stopTitle': 'Stop Container',
  'docker.restartTitle': 'Restart Container',
  'docker.startConfirm': 'Start container "{name}"?',
  'docker.stopConfirm': 'Stop container "{name}"? Services inside will be temporarily interrupted.',
  'docker.restartConfirm':
    'Restart container "{name}"? Services inside will be briefly interrupted.',
  'docker.startDone': 'Container {name} started',
  'docker.stopDone': 'Container {name} stopped',
  'docker.restartDone': 'Container {name} restarted',
  'docker.actionFailed': 'Container action failed',
  'docker.logsTitle': 'Container Logs - {name}',
  'docker.logsFailed': 'Failed to fetch logs',
  'docker.follow': 'Follow',
  'docker.composeTitle': 'Docker Compose One-Click Up',
  'docker.composeHint':
    'Select a docker-compose.yml / compose.yaml (or any .yml/.yaml) file on the remote server; docker compose up -d will be run in its directory.',
  'docker.startBtn': 'Start',
  'docker.running': 'Starting, pulling images may take a few minutes…',
  'docker.composeDone': 'Compose up finished',
  'docker.composeFailed': 'Compose up failed, check the output below',
  'docker.browseFailed': 'Failed to browse remote directory',
  'docker.noComposeFile':
    'No .yml / .yaml files in this directory. Enter a subdirectory or type a path above.',
  'docker.selectedFile': 'Selected',

  // Scheduled tasks (cron)
  'cron.add': 'New Task',
  'cron.colName': 'Task',
  'cron.colSchedule': 'Schedule',
  'cron.colCommand': 'Command',
  'cron.noJobs': 'No scheduled tasks',
  'cron.ctxHint': 'Right-click a task to edit / enable / disable / delete; double-click to edit',
  'cron.errRead': 'Failed to read crontab',
  'cron.errWrite': 'Failed to save crontab',
  'cron.newTitle': 'New Scheduled Task',
  'cron.editTitle': 'Edit Scheduled Task',
  'cron.name': 'Task Name',
  'cron.namePh': 'Optional, shown as a remark',
  'cron.schedule': 'Schedule',
  'cron.schedulePh': 'min hour day month weekday, e.g. 0 3 * * *',
  'cron.scheduleHint':
    'Format: min hour day month weekday (0-59 0-23 1-31 1-12 0-7), supports * , - /; or special expressions like @hourly',
  'cron.command': 'Command',
  'cron.commandPh': 'Command to execute',
  'cron.typeNMin': 'Every N minutes',
  'cron.typeNHour': 'Every N hours',
  'cron.typeDaily': 'Daily',
  'cron.typeWeekly': 'Weekly',
  'cron.typeMonthly': 'Monthly',
  'cron.typeYearly': 'Yearly',
  'cron.typeReboot': 'At reboot',
  'cron.typeCustom': 'Custom expression',
  'cron.segEvery': 'Every',
  'cron.uMinutes': 'minutes',
  'cron.uHours': 'hours',
  'cron.segAtMinute': 'at minute',
  'cron.segMinSuffix': '',
  'cron.segDaily': 'Daily at',
  'cron.segWeekly': 'Every',
  'cron.segMonthly': 'Every month on day',
  'cron.segDaySuffix': '',
  'cron.segYearly': 'Every year on',
  'cron.rebootHint': 'Runs once automatically after the system boots',
  'cron.deleteTitle': 'Delete Scheduled Task',
  'cron.deleteConfirm': 'Delete task "{name}"? This cannot be undone.',
  'cron.deleteDone': 'Task deleted',
  'cron.saved': 'Scheduled task saved',
  'cron.opWriting': 'Writing crontab…',
  'cron.enabled': 'Enabled',
  'cron.disabled': 'Disabled',
  'cron.enable': 'Enable',
  'cron.disable': 'Disable',
  'cron.enableDone': 'Task enabled',
  'cron.disableDone': 'Task disabled',
  'cron.errScheduleTitle': 'Check your input',
  'cron.errSchedule':
    'Invalid schedule: expected 5 fields (min hour day month weekday) or a special expression like @reboot',
  'cron.errCommand': 'Please enter the command to execute',
  'cron.pickScript': 'Pick a Script',
  'cron.pickTitle': 'Select a Script to Execute',
  'cron.pickHint':
    'Browse script files on the remote server; the command will be filled in automatically: .sh/.bash via bash, .py via python3, .js via node, other executables run directly.',
  'cron.noScript': 'No script files in this directory. Enter a subdirectory or type a path above.',
  'cron.browseFailed': 'Failed to browse remote directory',
  'cron.useScript': 'Use Script',
  'cron.selectedFile': 'Selected',

  // AI assistant
  'ai.newChat': 'New chat',
  'ai.untitled': 'New chat',
  'ai.noChats': 'No conversations yet',
  'ai.deleteChatConfirm': 'Delete this conversation?',
  'ai.notConfigured': 'Model not configured',
  'ai.send': 'Send',
  'ai.stop': 'Stop',
  'ai.inputPh': 'Ask anything…',
  'ai.inputDisabledPh': 'Configure an AI model in settings first',
  'ai.enterHint': 'Enter to send · Shift+Enter for newline',
  'ai.setupTitle': 'Configure AI Assistant',
  'ai.setupDesc':
    'Works with any OpenAI-compatible API: DeepSeek, OpenAI, Kimi, Qwen, GLM, Doubao and more. ' +
    'Pick a provider and enter your API key to start. The key is encrypted and stored locally.',
  'ai.goSetup': 'Configure',
  'ai.welcomeTitle': "Hi, I'm your AI ops assistant",
  'ai.welcomeDesc':
    'Ask me about Linux commands, scripting, error diagnosis, Docker or networking. ' +
    'Code blocks in replies can be copied with one click.',
  'ai.suggestDisk': 'How do I check disk usage on Linux?',
  'ai.suggestLargeFile': 'How do I find the largest files on the system?',
  'ai.suggestPort': 'How do I see processes listening on ports?',
  'ai.suggestCpu': 'How do I find the top CPU-consuming processes?',
  'ai.thinking': 'Thinking…',
  'ai.failed': 'Request failed',
  'ai.retry': 'Regenerate',
  'ai.copy': 'Copy',
  'ai.copied': 'Copied',
  'ai.copyCode': 'Copy code',
  'ai.copyMsg': 'Copy reply',
  'ai.providerCustom': 'Custom (OpenAI-compatible)',

  // Status bar
  'status.protocol': 'SSH2',
  'status.encoding': 'UTF-8',
  'status.cpu': 'CPU',
  'status.mem': 'Memory',
  'status.disk': 'Disk',

  // SFTP
  'sftp.local': 'Local Files',
  'sftp.remote': 'Remote Files',
  'sftp.filterAll': 'All',
  'sftp.filterDocs': 'Docs',
  'sftp.filterImages': 'Images',
  'sftp.filterArchives': 'Archives',
  'sftp.queue': 'Transfer Queue',
  'sftp.idle': 'Idle',
  'sftp.dragHint': 'Drag files here or use upload/download buttons',
  'sftp.pathPh': 'Type a path and press Enter',
  'sftp.mkdirPh': 'Folder name',
  'sftp.renamePh': 'Enter new name',
  'sftp.active': 'Active',
  'sftp.pending': 'Pending',
  'sftp.done': 'Done',
  'sftp.failed': 'Failed',
  'sftp.cancelled': 'Cancelled',
  'sftp.confirmDelete': 'Delete {count} selected item(s)? This cannot be undone.',
  'sftp.loadFailed': 'Failed to load directory',
  'sftp.opFailed': 'File operation failed',
  'sftp.opDeleting': 'Deleting…',
  'sftp.opCreatingFolder': 'Creating folder…',
  'sftp.opCreatingFile': 'Creating file…',
  'sftp.opRenaming': 'Renaming…',
  'sftp.opMoving': 'Moving…',
  'sftp.moveDone': 'Moved successfully',
  'sftp.deleteDone': 'Deleted successfully',
  'sftp.opExtracting': 'Extracting {name}…',
  'sftp.unzipMissingTitle': 'unzip not installed',
  'sftp.unzipMissingPrompt':
    'unzip was not detected on the server, .zip files cannot be extracted. Install it now?',
  'sftp.unzipInstallNow': 'Install now',
  'sftp.installingUnzip': 'Installing unzip…',
  'sftp.installUnzipTitle': 'Install unzip',
  'sftp.installUnzipDone': 'Installed successfully',
  'sftp.installUnzipFailed': 'Installation failed',
  'sftp.moveTitle': 'Move to remote folder',
  'sftp.moveSources': 'Move {count} selected item(s) to the destination folder:',
  'sftp.moveHere': 'Move here',
  'sftp.moveEmpty': 'This folder has no subfolders',
  'sftp.moveTarget': 'Destination: {dir}',
  'sftp.moveInvalid': "Invalid destination: can't move into itself or one of its subfolders",
  'sftp.uploadFailed': 'Upload failed',
  'sftp.downloadFailed': 'Download failed',
  'sftp.openFailed': 'Unable to open file',
  'sftp.notConnected': 'SSH session not connected',
  'sftp.up': 'Parent directory',
  'sftp.selected': '{count} selected',
  'sftp.clearFinished': 'Clear finished items',
  'sftp.cancel': 'Cancel transfer',
  'sftp.conflictTitle': 'File Conflict',
  'sftp.conflictRemote': 'Remote already has "{name}"',
  'sftp.conflictLocal': 'Local file already exists: "{name}"',
  'sftp.conflictDir': 'Target directory: {dir}',
  'sftp.overwrite': 'Overwrite',
  'sftp.conflictSkip': 'Skip',
  'sftp.applyAll': 'Apply this choice to all conflicts in this batch',
  'sftp.noSession': 'SSH session not connected, remote files unavailable',
  'sftp.size': 'Size',
  'sftp.mtime': 'Modified',
  'sftp.desktop': 'Desktop',

  // Settings
  'settings.title': 'Settings',
  'settings.general': 'General',
  'settings.terminal': 'Terminal',
  'settings.ssh': 'SSH',
  'settings.language': 'Language',
  'settings.languageDesc': 'Set the UI display language',
  'settings.restoreSession': 'Restore last session on startup',
  'settings.restoreSessionDesc': 'Reopen previous connections at launch',
  'settings.autoUpdate': 'Check for updates automatically',
  'settings.autoUpdateDesc': 'Check for new versions on startup',
  'settings.dataDir': 'Data directory',
  'settings.dataDirDesc': 'Where connections and config are stored',
  'settings.openDataDir': 'Open',
  'settings.fontFamily': 'Terminal font',
  'settings.fontSize': 'Font size',
  'settings.cursorStyle': 'Cursor style',
  'settings.cursorBlock': 'Block',
  'settings.cursorUnderline': 'Underline',
  'settings.cursorBar': 'Bar',
  'settings.cursorBlink': 'Cursor blink',
  'settings.scrollback': 'Scrollback lines',
  'settings.bell': 'Terminal bell',
  'settings.bellNone': 'Off',
  'settings.bellSound': 'Sound',
  'settings.editorMaxSize': 'Editor file limit (MB)',
  'settings.editorMaxSizeDesc': 'Maximum file size the remote text editor can open; files larger than this will be rejected',
  'settings.keepalive': 'Keep-alive interval (s)',
  'settings.keepaliveDesc': 'Sends periodic probes on idle connections to keep NAT/firewalls from dropping the link; 0 disables (not recommended)',
  'settings.timeout': 'Connect timeout (s)',
  'settings.compression': 'SSH compression',
  'settings.compressionDesc': 'Compress transferred data, useful on slow networks',
  'settings.autoReconnect': 'Auto-reconnect',
  'settings.autoReconnectDesc': 'Automatically retry after unexpected disconnects (up to 10 attempts)',
  'settings.transferConcurrency': 'Transfer concurrency',
  'settings.transferConcurrencyDesc':
    'Concurrent SFTP requests (1-64). If the server resets the connection during transfers, lower this to 1 (compatibility mode)',
  'settings.ai': 'AI Assistant',
  'settings.aiProvider': 'Provider',
  'settings.aiProviderDesc':
    'Pick a mainstream provider to auto-fill the endpoint, or choose custom for any OpenAI-compatible service',
  'settings.aiApiKey': 'API Key',
  'settings.aiApiKeyDesc':
    'Encrypted by the OS and stored in the local data directory; used only for model API calls',
  'settings.aiKeyPh': 'Enter API key',
  'settings.aiBaseUrl': 'Base URL',
  'settings.aiModel': 'Model',
  'settings.aiModelDesc': 'Model name; for Volcengine Ark etc. use the endpoint ID (ep-xxx)',
  'settings.aiSystemPrompt': 'System prompt',
  'settings.aiSystemPromptDesc':
    'Define the assistant role and answering style; applies to new questions',
  'settings.aiTest': 'Test connection',
  'settings.aiTestOk': 'Connection successful. Model reply: ',
  'settings.aiTestOkOnly': 'Connection successful',
  'settings.aiTestFail': 'Connection failed: ',
  'settings.aiErrTimeout':
    'Connection timed out. Requests use the system proxy automatically; make sure your proxy/VPN has system proxy enabled and can reach this endpoint',
  'settings.aiErrProxy':
    'Cannot reach the system proxy server. Check that your proxy app is running and the system proxy settings are correct',
  'settings.aiErrDns':
    'Cannot resolve the API host. Check the Base URL and your network/DNS settings',

  // Messages
  'msg.saved': 'Saved',
  'msg.deleted': 'Deleted',
  'msg.connected': 'Connected to {name}',
  'msg.disconnected': 'Disconnected',
  'msg.copied': 'Copied',
  'msg.nameRequired': 'Connection name is required',
  'msg.hostRequired': 'Host is required',
  'msg.invalidTarget': 'Invalid target format',
  'msg.opFailed': 'Operation failed: {msg}',
  'msg.fileExists': '{name} already exists',
  'msg.deleteGroupConfirm': 'Delete group "{name}"? Connections will move to ungrouped.',
  'msg.deleteConnConfirm': 'Delete connection "{name}"?',
  'msg.restoreFailed': 'Failed to restore session: {msg}',

  'update.title': 'Software Update',
  'update.checking': 'Checking for updates...',
  'update.available': 'New version {version} available',
  'update.availableHint': 'You will be prompted when the download completes',
  'update.latest': 'You are on the latest version',
  'update.download': 'Download Update',
  'update.later': 'Later',
  'update.downloading': 'Downloading update',
  'update.downloaded': '{version} downloaded',
  'update.restart': 'Restart & Install',
  'update.restartHint': 'The app will restart to finish installing',
  'update.failed': 'Update check failed',
  'update.failedSilent': 'Automatic update check failed. You can retry later.',
  'update.check': 'Check for Updates',
  'update.unavailableInDev': 'Auto-update is unavailable in development (works when packaged)',
} as const

export type DictKey = keyof typeof import('./en-US').default
